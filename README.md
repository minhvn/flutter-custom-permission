...
[![pub package](https://img.shields.io/pub/v/custom_permission.svg)](https://pub.dartlang.org/packages/custom_permission)
...
To use this plugin, add `custom_permission` as a [dependency in your pubspec.yaml file](https://flutter.io/platform-plugins/). For example:

```yaml
dependencies:
  custom_permission: '^3.2.0'
```

> **NOTE:** As of version 3.1.0 the custom_permission plugin switched to the AndroidX version of the Android Support Libraries. This means you need to make sure your Android project is also upgraded to support AndroidX. Detailed instructions can be found [here](https://flutter.dev/docs/development/packages-and-plugins/androidx-compatibility).
>
...
```dart
import 'package:custom_permission/custom_permission.dart';

Map<PermissionGroup, PermissionStatus> permissions = await CustomPermission().requestPermissions([PermissionGroup.contacts]);
```
...
PermissionStatus permission = await CustomPermission().checkPermissionStatus(PermissionGroup.contacts);
```
...
ServiceStatus serviceStatus = await CustomPermission().checkServiceStatus(PermissionGroup.location);
```
...
bool isOpened = await CustomPermission().openAppSettings();
```
...
bool isShown = await CustomPermission().shouldShowRequestPermissionRationale(PermissionGroup.contacts);
```
